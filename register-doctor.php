...
include 'database.php';
   ...
   $select = mysqli_query($conn, "SELECT * FROM `receint-doctor` WHERE name = '$name' AND email = '$email'") or 
   die('query failed');
  ...
  if(mysqli_num_rows($select) > 0){
     $message[] = 'user already exist'; 
      ...
      $insert = mysqli_query($conn, "INSERT INTO `receint-doctor`(name, email,password, address,department,status) VALUES('$name', '$email','$password', '$pass','$department','approved')") or die('query failed');
        
                 if($insert){
                    move_uploaded_file($image_tmp_name, $image_folder);
                    $message[] = 'registered successfully!';
                    header('location:doctor-record.php');
                 }else{
