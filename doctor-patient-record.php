...
<div class="right-side">
  ...
  <?php 
     include 'database.php';
    $sql="SELECT * from `appointment_list` where Doctor_name='$name12'";
    $result=mysqli_query($conn,$sql);
    if($result){
    while($row=mysqli_fetch_assoc($result)){
       $id=$row["Id"]; 
       $name=$row["patient_name"]; 
       $symptom=$row["descripition"]; 
       $email=$row["date"];
       $appoint=$row["appoint"];
       echo"
       <tr>
      ...
      <td>$appoint</td>
      <td>
       <button class='btn btn-danger'><a href='doctor-patient-delete.php?id=$id'class='text-light'>Treated</a></button>
      <td>
    </tr> 
       ";
